...
# 'Disconnect PowerShell Script'
readonly DISCONNECT_PS_SCRIPT_PATH="./install/winapps-disconnect.ps1"                              # UNIX path to the disconnect wrapper PowerShell script.
readonly DISCONNECT_PS_HOME_PATH="${USER_APPDATA_PATH}/winapps-disconnect.ps1"                      # UNIX path to a copy of the disconnect script within the user's home directory.
readonly DISCONNECT_PS_HOME_PATH_WIN="${USER_APPDATA_PATH_WIN}\\winapps-disconnect.ps1"             # WINDOWS path to a copy of the disconnect script within the user's home directory.
# 'Detected File'
...
function waFindInstalled() {
    ...
    # Copy disconnect wrapper script to a directory within the user's home folder.
    cp "$DISCONNECT_PS_SCRIPT_PATH" "$DISCONNECT_PS_HOME_PATH"
    ...
    # Deploy the disconnect wrapper script to a stable Windows-local path.
    echo "IF NOT EXIST \"C:\\ProgramData\\WinApps\" mkdir \"C:\\ProgramData\\WinApps\"" >>"$BATCH_SCRIPT_PATH"
    echo "copy /Y \"${DISCONNECT_PS_HOME_PATH_WIN}\" \"C:\\ProgramData\\WinApps\\winapps-disconnect.ps1\"" >>"$BATCH_SCRIPT_PATH"
